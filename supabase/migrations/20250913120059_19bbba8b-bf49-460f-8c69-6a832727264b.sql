-- Drop and recreate the authenticate_admin function with proper digest syntax
...
BEGIN
  ...
  -- Use proper casting for digest function
  input_hash := encode(digest((input_username || input_password)::bytea, 'sha256'), 'hex');
...
-- Update the admin password hash with proper casting
UPDATE public.admins 
SET password_hash = encode(digest(('admin' || '151515')::bytea, 'sha256'), 'hex')
