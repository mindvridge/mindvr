-- Fix the authenticate_admin function with proper digest parameter types
...
BEGIN
  ...
  -- Use convert_to instead of casting to bytea for proper encoding
  input_hash := encode(digest(convert_to(input_username || input_password, 'UTF8'), 'sha256'), 'hex');
...
-- Update the admin password hash with proper encoding
UPDATE public.admins 
SET password_hash = encode(digest(convert_to('admin' || '151515', 'UTF8'), 'sha256'), 'hex')
